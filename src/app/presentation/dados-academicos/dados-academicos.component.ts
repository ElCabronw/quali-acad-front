...
export class DadosAcademicosComponent implements OnInit {
  ...
  async filterDadosAcademicos(event: any) {
    this.data = await this.getDadosAcademicosUseCase.getDadosAcademicosFilter(
      event
    );
  }
  async filterDadosAcademicosChart(event: any) {
    this.chartData =
      await this.getDadosAcademicosUseCase.getDadosAcademicosChartView(event);
  ...
  async ngAfterViewInit() {
    ...
    console.log('geoMapContainer:', this.geoMapContainer);
    if (this.geoMapContainer && this.geoMapContainer.nativeElement) {
      console.log(
        'geoMapContainer nativeElement:',
        this.geoMapContainer.nativeElement
      );
      console.log(
        'geoMapContainer width:',
        this.geoMapContainer.nativeElement.offsetWidth
      );
      console.log(
        'geoMapContainer height:',
        this.geoMapContainer.nativeElement.offsetHeight
      );
      ...
      await this.filterDadosAcademicosChart({
        anoAvaliacao: new Date().getFullYear(),
      });
    } else {
      console.warn(
        'geoMapContainer or its nativeElement is not available in ngAfterViewInit.'
      );
    }
  }
  ...
  private _buildBarChartStars() {
    ...
    this.chartBarOptions = {
      ...
      onClick: (event: any, elements: any[]) =>
        this.handleChartStartsClick(event, elements),
      plugins: {
        legend: {
          labels: {
            color: textColor,
  ...
  private _buildSummaryPieChart() {
    ...
    this.estreladosChart = {
      ...
      datasets: [
        {
          data: [
            this.chartData.countEstrelados,
            this.chartData.countNaoEstrelados,
          ],
          backgroundColor: [
            documentStyle.getPropertyValue('--primary-color'),
            documentStyle.getPropertyValue('--red-400'),
